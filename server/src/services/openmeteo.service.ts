...
export interface OpenMeteoDailySunlight {
  date: string;
  timezone: string;
  sunrise: string;
  sunset: string;
...
class OpenMeteoService {
  ...
  async fetchDailySunlight(lat: number, lon: number, timezone: string): Promise<OpenMeteoDailySunlight> {
    const params = new URLSearchParams({
      latitude: lat.toString(),
      longitude: lon.toString(),
    ...
    const data = (await response.json()) as OpenMeteoResponse;
    return this.parseDailySunlightResponse(data, timezone);
  }

  private parseHourlyResponse(data: OpenMeteoResponse): OpenMeteoPoint[] {
    if (!data.hourly) {
      throw new Error('Open-Meteo hourly response is missing hourly data');
    }
  ...
  private parseDailySunlightResponse(data: OpenMeteoResponse, requestedTimezone: string): OpenMeteoDailySunlight {
    const daily = data.daily;
    if (!daily?.time?.length || !daily.sunrise?.length || !daily.sunset?.length || !daily.daylight_duration?.length) {
      throw new Error('Open-Meteo daily response is missing sunlight data');
    }

    const sunrise = daily.sunrise[0];
    const sunset = daily.sunset[0];
    const daylightDuration = daily.daylight_duration[0];
    if (!sunrise || !sunset || daylightDuration == null) {
      throw new Error('Open-Meteo daily response contains incomplete sunlight data');
    }
    ...
    return {
      ...
      sunrise,
      sunset,
      daylightHours: daylightDuration / 3600,
      sunshineHours: sunshineDuration == null ? null : sunshineDuration / 3600,
    };
  }
}
