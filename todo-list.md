## Domingo 08-03 

### To-Do

- Revisar que cuando se expande el menú, o se contrae, se guarde esa configuracion al moverse entre pantallas.
- [DONE] Cambiar la pantalla de My Projects para que aproveche todo el ancho, como lo hacen la de dashboard y la de paneles
- [DONE] Modificar las páginas de settings y de usuario para que se parezcan a las de dashboard y my projects.

### Puede esperar

- Mordernizar la vista de un proyecto (view-project)

### Importantes para la proxima reunión

- Implementar la nueva version de la pantalla de crear proyecto (debería seguir la idea de que se use el ancho de la pantalla.)
- Revisar los cálculos que se hacen y que se muestran y ver que estén todos completos (uno de los requisitos de la ultima reunion)

### Para las features de admin
- Ver si es posible usar un único layout para los dos tipos de usuario (user y admin)
  - Y que en función del tipo de usuario se muestren/oculten pantallas
- Ver si en ciertas pantallas que son esencialmente las mismas, puedo también mostrar botones, información sólo si eres admin.

- Esto de mostrar y ocultar me vendría bien para la vista de view-project, podría reutilizar lo que tengo en configure project y que la vista de view-project sea una especie de read-only de configure project.
  - Obviamente haría falta añadir algunos datos más para la vista de view-project, pero creo que la idea es buena y me evito crear mil pantallas y diseños diferentes.
